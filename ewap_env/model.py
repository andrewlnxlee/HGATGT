...
class GNNGroupTracker(nn.Module):
    def __init__(self, input_node_dim=config.INPUT_DIM, input_edge_dim=config.EDGE_DIM, hidden_dim=96): 
        super(GNNGroupTracker, self).__init__()
        
        # 1. 增强型输入编码
        # 核心修复：傅里叶编码器固定只处理 2 维坐标 [x, y]
        self.fourier_dim = 64
        self.pos_encoder = FourierFeatureEncoder(2, self.fourier_dim // 2, scale=2.0)
    ...
    def forward(self, data):
        ...
        # 仅对坐标部分 [x, y] 进行傅里叶编码
        x_pos = x[:, :2]
        x_fourier = self.pos_encoder(x_pos) # [N, 64]
        # 拼接原始坐标 (包含速度)
        x_in = torch.cat([x, x_fourier], dim=1) 
