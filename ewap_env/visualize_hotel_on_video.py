...
# --- 坐标转换与校准 ---
def world_to_pixel(x, y, H_inv, frame_height, u_offset=50, v_offset=-50, flip_y=False):
    ...
    # Hotel 场景直接复用 ETH 的映射时，容易出现垂直方向颠倒。
    # 这里按视频帧高度做一次 y 轴镜像，保证投影结果与图像坐标系一致。
    if flip_y:
        v = frame_height - 1 - v

    return int(round(u + u_offset)), int(round(v + v_offset))
...
def visualize_hotel_on_video():
    ...
    save_path = os.path.join(config.OUTPUT_MP4_DIR, "hotel_v1.mp4")
    device = torch.device(config.DEVICE if torch.cuda.is_available() else "cpu")

    # Hotel 需要单独开启竖直翻转修正，避免投影到视频上时上下翻转。
    flip_y = True
    ...
    print(f"正在生成 {scene_name} 高清演示视频并统计准确率 (共 {len(sorted_frames[:1500])} 采样点)...")
    ...
    print("\n" + "=" * 50 + f"\n📊 H-GAT 群体划分性能统计报告 ({scene_name} Dataset)\n" + "=" * 50)
